...
$requete = faireUneRequeteOnLine_Crawltrack($sql, $crawltconnexion);
...
$requetecrawltconfig = faireUneRequeteOnLine_Crawltrack($sqlcrawltconfig, $crawltconnexion);
...
$crawltresult = faireUneRequeteOnLine_Crawltrack("SELECT id FROM crawlt_hits
...
if ($crawltnum_rows > 0) {
    ...
    $crawltrequeteupdate = faireUneRequeteOnLine_Crawltrack($crawltsqlupdate, $crawltconnexion);
} else {
    ...
    $crawltrequete = faireUneRequeteOnLine_Crawltrack($crawltsql, $crawltconnexion);
}
// ---------------------------------------------------------------------
...
if ($crawlttypeattack != 0 && $crawltattack == 1) {
    ...
    if ($crawlthttpcode == 404) {
        ...
        $crawltresult = faireUneRequeteOnLine_Crawltrack("SELECT id FROM crawlt_error
		WHERE  date='" . crawlt_sql_quote($crawlttodaylocal) . "'
        ...
        $result2 = faireUneRequeteOnLine_Crawltrack("SELECT id_page FROM crawlt_pages_attack WHERE url_page='" . crawlt_sql_quote($crawlturl) . "'", $crawltconnexion);
        ...
        if ($num_rows2 > 0) {
            ...
            faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_pages_attack (url_page) VALUES ('" . crawlt_sql_quote($crawlturl) . "')", $crawltconnexion);
            $crawltid_insert = exploiterLigneResultatBDD_Crawltrack_row(faireUneRequeteOnLine_Crawltrack("SELECT LAST_INSERT_ID()", $crawltconnexion));
            $crawltpage = $crawltid_insert[0];
        }
        // insertion of the visit datas in the visits database
        faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_visits (crawlt_site_id_site, crawlt_pages_id_page, crawlt_crawler_id_crawler, date, crawlt_ip_used, crawlt_error) VALUES ('" . crawlt_sql_quote($crawltsite) . "', '" . crawlt_sql_quote($crawltpage) . "', '" . crawlt_sql_quote($crawlttypeattack) . "', '" . crawlt_sql_quote($crawltdate) . "', '" . crawlt_sql_quote($crawltip) . "','0')", $crawltconnexion);
    }
} else {
    ...
    $result = faireUneRequeteOnLine_Crawltrack("SELECT crawler_user_agent, crawler_ip,id_crawler FROM crawlt_crawler
    ...
    if ($num_rows > 0) {
        ...
        $crawltdate = date("Y-m-d H:i:s");
        // check if the page already exist, if not add it to the table
        $result2 = faireUneRequeteOnLine_Crawltrack("SELECT id_page FROM crawlt_pages WHERE url_page='" . crawlt_sql_quote($crawlturl) . "'", $crawltconnexion);
        $num_rows2 = exploiterNombreLigneResultatBDD_Crawltrack($result2);
        if ($num_rows2 > 0) {
            $crawltdata2 = exploiterLigneResultatBDD_Crawltrack_row($result2);
            $crawltpage = $crawltdata2[0];
        } else {
            faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_pages (url_page) VALUES ('" . crawlt_sql_quote($crawlturl) . "')", $crawltconnexion);
            $crawltid_insert = exploiterLigneResultatBDD_Crawltrack_row(faireUneRequeteOnLine_Crawltrack("SELECT LAST_INSERT_ID()", $crawltconnexion));
            $crawltpage = $crawltid_insert[0];
        }
        if ($crawlthttpcode == 404) {
        ...
        if ((! isset($_COOKIE["crawltrackstats" . $crawltsite]) || $_COOKIE["crawltrackstats" . $crawltsite] != 'nocountinstats') && (ereg($crawltbrowserconcat, $crawltagent) && ! ereg($crawltnonebrowserconcat, $crawltagent))) {
            ...
            if ($crawltrefereok == 1) {
                ...
                if ($crawltsearchengine != 0) {
                    ...
                    $result5 = faireUneRequeteOnLine_Crawltrack("SELECT id_referer FROM crawlt_referer WHERE referer='" . crawlt_sql_quote($crawltreferer) . "'", $crawltconnexion);
                    ...
                    if ($num_rows5 > 0) {
                        ...
                        faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_referer (referer) VALUES ('" . crawlt_sql_quote($crawltreferer) . "')", $crawltconnexion);
                        $crawltid_insert3 = exploiterLigneResultatBDD_Crawltrack_row(faireUneRequeteOnLine_Crawltrack("SELECT LAST_INSERT_ID()", $crawltconnexion));
                        $crawltrefererid = $crawltid_insert3[0];
                    }
                    // check if the keyword already exist, if not add it to the table
                    $result4 = faireUneRequeteOnLine_Crawltrack("SELECT id_keyword FROM crawlt_keyword WHERE keyword='" . crawlt_sql_quote($crawltkeyword) . "'", $crawltconnexion);
                    ...
                    if ($num_rows4 > 0) {
                        ...
                        faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_keyword (keyword) VALUES ('" . crawlt_sql_quote($crawltkeyword) . "')", $crawltconnexion);
                        $crawltid_insert2 = exploiterLigneResultatBDD_Crawltrack_row(faireUneRequeteOnLine_Crawltrack("SELECT LAST_INSERT_ID()", $crawltconnexion));
                    ...
                    faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_visits_human (crawlt_site_id_site, crawlt_keyword_id_keyword, crawlt_id_crawler, date, crawlt_id_page, crawlt_id_referer, crawlt_ip, crawlt_error, crawlt_browser) VALUES ('" . crawlt_sql_quote($crawltsite) . "', '" . crawlt_sql_quote($crawltkeywordid) . "', '" . crawlt_sql_quote($crawltsearchengine) . "', '" . crawlt_sql_quote($crawltdate) . "', '" . crawlt_sql_quote($crawltpage) . "','" . crawlt_sql_quote($crawltrefererid) . "','" . crawlt_sql_quote($crawltip) . "', '" . crawlt_sql_quote($crawlterror) . "','" . crawlt_sql_quote($crawltbrowser) . "')", $crawltconnexion);
                } else {
                    // case other referer
                    // check if the referer already exist, if not add it to the table
                    $result5 = faireUneRequeteOnLine_Crawltrack("SELECT id_referer FROM crawlt_referer WHERE referer='" . crawlt_sql_quote($crawltreferer) . "'", $crawltconnexion);
                ...
                faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_visits_human (crawlt_site_id_site, crawlt_keyword_id_keyword, crawlt_id_crawler, date, crawlt_id_page, crawlt_id_referer, crawlt_ip, crawlt_error, crawlt_browser) VALUES ('" . crawlt_sql_quote($crawltsite) . "', '0', '0','" . crawlt_sql_quote($crawltdate) . "', '" . crawlt_sql_quote($crawltpage) . "','0','" . crawlt_sql_quote($crawltip) . "', '" . crawlt_sql_quote($crawlterror) . "','" . crawlt_sql_quote($crawltbrowser) . "')", $crawltconnexion);
            }
        }
    }
}
...
$url_crawlt = "http://localhost/16.B25/B25.fr/besancon25 - La plate-Forme des artistes v5.9/scriptPHP/crawltrack3-3-2/";
if (($crawltdatetoday != $crawltdatelastseorequest) && $crawltcheck == 1) {
    $crawltpath = "/srv/www/16.B25/B25.fr/besancon25 - La plate-Forme des artistes v5.9/scriptPHP/crawltrack3-3-2/include";
...
if ($crawltattack == 1 && $crawltblockattack == 1 && $crawltpostrequest == 1) {
    ...
    echo "<head>";
    echo "<META HTTP-EQUIV='Refresh' CONTENT='0;URL=http://localhost/16.B25/B25.fr/besancon25 - La plate-Forme des artistes v5.9/scriptPHP/crawltrack3-3-2/html/noacces.php'>";
    echo "</head>";
}
?>
