...
if ($validlogin == 1) {
    if (empty($login) || empty($password2) || empty($password3) || $password2 != $password3) {
        ...
        $nbrresult = exploiterNombreLigneResultatBDD_Crawltrack($queryexist);
        if ($nbrresult >= 1) {
            // login already exist
            ...
            $querylogin = faireUneRequeteOnLine_Crawltrack($sqllogin, $connexion) or die("MySQL query error");
    ...
    $querysite = faireUneRequeteOnLine_Crawltrack($sqlsite, $connexion);
    $nbrresult = exploiterNombreLigneResultatBDD_Crawltrack($querysite);
    while ($ligne = exploiterLigneResultatBDD_Crawltrack($querysite)) {
        $sitename = $ligne['name'];
