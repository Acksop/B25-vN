...
if ($suppressuser == 1) {
	...
	$requetedeleteuser = faireUneRequeteOnLine_Crawltrack($sqldeleteuser, $connexion);
	$nbrresult = exploiterNombreLigneResultatBDD_Crawltrack($requetedeleteuser);
	if ($nbrresult >= 1) {
		while ($ligne = exploiterLigneResultatBDD_Crawltrack($requetedeleteuser)) {
