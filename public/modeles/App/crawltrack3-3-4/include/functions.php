...
if (! class_exists('nanoSha2')) {
    ...
    class nanoSha2
    {
         ...
         * @param $str Input
         *            string to hash
         * @param $ig_func Option
         *            param to ignore checking for php > 5.1.2
...
function cache($cachename)
{
    ...
    $nbrresult = exploiterNombreLigneResultatBDD_Crawltrack($requetecache);
    if ($nbrresult >= 1) {
        $ligne = exploiterLigneResultatBDD_Crawltrack_row($requetecache);
        $time = $ligne[0][0];
    } else {
        $time = 0;
    }
    ...
    if (file_exists($pathFromSiteIndexToCrawltrackIndex . "cache/$cachename.txt") && ($time + 3600) > time() && $nocachetest != 1) {
        ...
        if ($time == 0) {
            ...
            $requetecache2 = faireUneRequeteOnLine_Crawltrack($sqlcache2, $connexion);
        } else {
            $timecache = time();
            $sqlcache3 = "UPDATE crawlt_cache SET time='$timecache' where cachename='$cachename'";
            $requetecache3 = faireUneRequeteOnLine_Crawltrack($sqlcache3, $connexion);
        }
        $caching = 'true';
        ob_start();
    }
