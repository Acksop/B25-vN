    ...
    if ($period == 3) // case one year
{
        ...
        $requetereferer = faireUneRequeteOnLine_Crawltrack($sqlreferer, $connexion);
        
        while ($ligne = exploiterLigneResultatBDD_Crawltrack_row($requetereferer)) {
        ...
        $sqldirect = "SELECT FROM_UNIXTIME(UNIX_TIMESTAMP(`date`)-($times*3600), '%m\/%Y'), COUNT(DISTINCT CONCAT(crawlt_ip, crawlt_browser))
			...
			GROUP BY FROM_UNIXTIME(UNIX_TIMESTAMP(`date`)-($times*3600), '%m\/%Y')";
        $requetedirect = faireUneRequeteOnLine_Crawltrack($sqldirect, $connexion);
        while ($ligne = exploiterLigneResultatBDD_Crawltrack_row($requetedirect)) {
...
$nbrpage = exploiterLigneResultatBDD_Crawltrack(faireUneRequeteOnLine_Crawltrack($sql, $connexion));
$nbrpage = $nbrpage["numrow"];
