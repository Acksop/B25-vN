...
{
    $result = faireUneRequeteOnLine_Crawltrack("UPDATE crawlt_update SET update_id='97' WHERE idcrawlt_update='1'");
    if ($result) {
        if (exploiterNombreLigneResultatBDD_Crawltrack($result) == 0) {
            faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_update VALUES (1,'97')");
        }
    }
}
...
{
    $result = faireUneRequeteOnLine_Crawltrack("UPDATE crawlt_update_attack SET update_id='8' WHERE idcrawlt_update='1'");
    if ($result) {
        if (exploiterNombreLigneResultatBDD_Crawltrack($result) == 0) {
            faireUneRequeteOnLine_Crawltrack("INSERT INTO crawlt_update_attack VALUES (1,'8')");
        }
    }
}
...
$tables = faireUneRequeteOnLine_Crawltrack($tables_list_sql, $connexion) or exit("MySQL query error");

$tables_names = array();
while (list ($tablename) = exploiterLigneResultatBDD_Crawltrack($tables)) {
...
if ($maintenance_mode == 'install') {
    ...
    foreach ($tables_to_check as $table_to_check) {
        if ($table_to_check['action'] == 'create') {
            ...
            if (! in_array($table_to_check['table_name'], $tables_names)) {
                ...
                $result_create = faireUneRequeteOnLine_Crawltrack($table_to_check['create_delete_query'], $connexion);
                if (! $result_create) {
                    // Query failed, add the error message
                    $tables_actions_error_messages[] = "ERROR ON QUERY !";
                }
                ...
                if (! empty($table_to_check['insert_query'])) {
                    ...
                    if (strpos($table_to_check['insert_query'], 'INSERT') !== false) {
                        $result_insert = faireUneRequeteOnLine_Crawltrack($table_to_check['insert_query'], $connexion);
                    } else {
                        // use the SQL file in data directory
                        $result_insert = faireUneRequeteOnLine_Crawltrack(file_get_contents(dirname(__FILE__) . '/data/' . $table_to_check['insert_query']), $connexion);
                    }
                    if (! $result_insert) {
                        // Query failed, add the error message
                        $tables_actions_error_messages[] = "ERROR ON QUERY !";
    ...
    foreach ($tables_to_check as $table_to_check) {
        if ($tables_to_touch == 'all' || (is_array($tables_to_touch) && in_array($table_to_check['table_name'], $tables_to_touch))) {
            ...
            if ($table_to_check['action'] == 'create') {
                ...
                $result_delete = faireUneRequeteOnLine_Crawltrack($table_to_check['create_delete_query'], $connexion);
                if (! $result_delete) {
                    // Query failed, add the error message
                    $tables_actions_error_messages[] = "ERROR ON QUERY !";
                }
    ...
    foreach ($fields_to_check as $table_name => $fields) {
        if ($tables_to_touch == 'all' || (is_array($tables_to_touch) && in_array($table_name, $tables_to_touch))) {
            // Either all the tables are to be modified or the current table is in the restricted tables array
            $table_info_res = faireUneRequeteOnLine_Crawltrack("SHOW COLUMNS FROM " . $table_name . "");
            $table_field_names = array();
            while ($table_info = exploiterLigneResultatBDD_Crawltrack($table_info_res)) {
            ...
            foreach ($fields as $a_field) {
                if (! in_array($a_field['field_name'], $table_field_names)) {
                    ...
                    $result_update = faireUneRequeteOnLine_Crawltrack($a_field['add_query'], $connexion);
                    if (! $result_update) {
                        // Query failed, add the error message
                        $fields_actions_error_messages[] = "ERROR ON QUERY !";
                    }
                }
            }
        }
    ...
    foreach ($indexes_to_check as $table_name => $indexes) {
        if ($tables_to_touch == 'all' || (is_array($tables_to_touch) && in_array($table_name, $tables_to_touch))) {
            // Either all the tables are to be modified or the current table is in the restricted tables array
            $table_info_res = faireUneRequeteOnLine_Crawltrack("SHOW INDEX FROM " . $table_name . "");
            $table_field_names = array();
            while ($table_info = exploiterLigneResultatBDD_Crawltrack($table_info_res)) {
            ...
            foreach ($indexes as $an_index) {
                if (! in_array($an_index['index_name'], $table_index_names)) {
                    // The field isn't in the existing tables list, create it
                    $result_update = faireUneRequeteOnLine_Crawltrack($an_index['add_query'], $connexion);
                    if (! $result_update) {
                        // Query failed, add the error message
                        $index_actions_error_messages[] = "ERROR ON QUERY !";
                    }
                }
            }
        }
