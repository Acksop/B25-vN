...
if ($validlogin == 1) {
    if (empty($crawlername2) || (empty($crawlerua2) && empty($crawlerip2)) || empty($crawleruser2) || empty($crawlerurl2)) {
        ...
        if ($nbrresult >= 1) {
            $ligne = exploiterLigneResultatBDD_Crawltrack($queryexist);
            ...
            $querycrawler = faireUneRequeteOnLine_Crawltrack($sqlcrawler, $connexion);
            
            // empty the cache table
            $sqlcache = "TRUNCATE TABLE crawlt_cache";
            $querycache = faireUneRequeteOnLine_Crawltrack($sqlcache, $connexion);
