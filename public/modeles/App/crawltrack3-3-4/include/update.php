...
<br>
<br>
<h1><?php echo $language['update_title_attack'] ?></h1>

<h2><?php echo $language['your_list'] ?>&nbsp;Crawlerlist <?php echo $idlastupdate ?></h2>
<h2><?php echo $language['crawltrack_list'] ?>&nbsp;<iframe name="I1"
		src="http://www.crawltrack.net/listcrawler/infolistid.htm"
		marginwidth="1" marginheight="1" scrolling="no" border="0"
		frameborder="0" width="150px" height="24px"></iframe>
</h2>
...
<div class="form">
	...
	<h2>
		<form
			...
			method="POST">
			<input type="hidden" name="validform" value="14"> <input
				type="hidden" name="navig" value="6"> <input name='ok' type="submit"
				value="<?php echo $language['update_crawler'] ?>" size="20">
		</form>
		<br>
		...
		<form
			action="index.php<?php echo $varPostFormIncludePageWithRedirection;?>"
			method="POST">
			<input type="hidden" name="navig" value="6"> <input name="ok"
				type="submit" value="<?php echo $language['no_update'] ?>" size="20">
		</form>
	</h2>
