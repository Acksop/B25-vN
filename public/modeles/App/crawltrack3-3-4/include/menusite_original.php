...
if ($_SESSION['rightsite'] == 0) {
        ...
        if ($navig == 2 || $navig == 3) {
            if (! isset($_SESSION[$sitename2])) {
                ...
                $sqlstats = "SELECT COUNT(DISTINCT crawlt_pages_id_page) as numrow FROM crawlt_visits
					...
					";
                $nbrpagestotal = exploiterNombreLigneResultatBDD_Crawltrack(faireUneRequeteOnLine_Crawltrack($sqlstats));
    ...
    if ($nbrresult >= 1) {
        ...
        if (! isset($_SESSION[$sitename])) {
            ...
            $nbrpagestotal = exploiterNombreLigneResultatBDD_Crawltrack($requetestats);
