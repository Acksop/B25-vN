...
$requetecrawltupdatemail = faireUneRequeteOnLine_Crawltrack($sqlcrawltupdatemail, $crawltconnexion);
...
if (function_exists("faireUneRequeteOnLine_Crawltrack") == false) {

    function faireUneRequeteOnLine_Crawltrack($sql)
    {
        global $numbquery;
        $numbquery ++;
        return faireUneRequeteOnLine_Crawltrack($sql);
    }
}
...
$requetecrawltsite = faireUneRequeteOnLine_Crawltrack($sqlcrawltsite, $crawltconnexion);
$nbrresultcrawlt2 = exploiterNombreLigneResultatBDD_Crawltrack($requetecrawltsite);
if ($nbrresultcrawlt2 >= 1) {
    $listsitecrawlt = array();
    $crawltsitename = array();
    $crawltsiteurl = array();
    while ($lignecrawlt2 = exploiterLigneResultatBDD_Crawltrack($requetecrawltsite)) {
...
foreach ($listsitecrawlt as $site) {
    ...
    $sqlstats2 = "SELECT COUNT(DISTINCT crawlt_pages_id_page), COUNT(DISTINCT crawler_name), COUNT(id_visit) 
		...
		AND $datetolookfor         
		AND crawlt_visits.crawlt_site_id_site='" . crawlt_sql_quote($site) . "'";
    $requetestats2 = faireUneRequeteOnLine_Crawltrack($sqlstats2, $crawltconnexion);
    ...
    $requeteseo = faireUneRequeteOnLine_Crawltrack($sqlseo, $crawltconnexion);
...
if ($crawltracklite == 1) {
    ...
    $requete = faireUneRequeteOnLine_Crawltrack($sqldelete, $crawltconnexion);
    ...
    $requeteoptimize2 = faireUneRequeteOnLine_Crawltrack($sqloptimize2, $crawltconnexion);
    ...
    if ($nbrresult >= 1) {
        ...
        $requetedelete2 = faireUneRequeteOnLine_Crawltrack($sqldelete2, $crawltconnexion);
