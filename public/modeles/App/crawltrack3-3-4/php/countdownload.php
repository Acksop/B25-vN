...
$result = faireUneRequeteOnLine_Crawltrack("SELECT id_site FROM crawlt_site
...
if ($num_rows > 0) {
    ...
    while ($ligne = exploiterLigneResultatBDD_Crawltrack_row($result)) {
        $idsite = $ligne[0];
    }
} elseif ($noconnect == 0) {
    exit('<h1>Hacking attempt !!!!</h1>');
...
if ($count_download) {
    ...
    $result = faireUneRequeteOnLine_Crawltrack("SELECT id FROM crawlt_download
    ...
    connectionBDD_Crawltrack($connexion);
