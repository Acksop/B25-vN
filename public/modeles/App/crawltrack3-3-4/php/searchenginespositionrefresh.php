...
$requetecrawltconfig = faireUneRequeteOnLine_Crawltrack($sqlcrawltconfig, $connexion);
...
$crawltrequete = faireUneRequeteOnLine_Crawltrack($crawltsql, $connexion);
$crawltnbrresult = exploiterNombreLigneResultatBDD_Crawltrack($crawltrequete);
...
if ($crawltnbrresult >= 1) {
    while ($crawltligne = exploiterLigneResultatBDD_Crawltrack_row($crawltrequete)) {
...
if ($retry == 'yahoo') {
    ...
    foreach ($listsitecrawlt as $crawltidsite) {
        ...
        $crawltrequetecheck = faireUneRequeteOnLine_Crawltrack($crawltsqlcheck, $connexion);
        $crawltnbrresultcheck = exploiterNombreLigneResultatBDD_Crawltrack($crawltrequetecheck);
        if ($crawltnbrresultcheck >= 1) {
            $crawltligne = exploiterLigneResultatBDD_Crawltrack($crawltrequetecheck);
            ...
            if ($crawltexitingvalue == 0) {
				...
				WHERE `date`= '" . sql_quote($crawltdatetoday2) . "'
				AND id_site='" . sql_quote($crawltidsite) . "'";
                $crawltrequeteseo = faireUneRequeteOnLine_Crawltrack($crawltsqlseo, $connexion);
            }
        } else {
    ...
    foreach ($listsitecrawlt as $crawltidsite) {
        ...
        $crawltsearchresults1 = $soapclient->call("Search", array(
            "Request" => $crawltparam1
        ));
        ...
        if ($crawltnbrresultcheck >= 1) {
            ...
            if ($crawltexitingvalue == 0) {
				...
				WHERE date= '" . sql_quote($crawltdatetoday2) . "'
				AND id_site='" . sql_quote($crawltidsite) . "'";
                $crawltrequeteseo = faireUneRequeteOnLine_Crawltrack($crawltsqlseo, $connexion) or die("MySQL query error");
            }
        } else {
        ...
        $crawltsearchresults2 = $soapclient->call("Search", array(
            "Request" => $crawltparam2
        ));
    ...
    foreach ($listsitecrawlt as $crawltidsite) {
        ...
        $crawltsqlcheck = "SELECT `date`, id_site, nbrdelicious  FROM crawlt_seo_position
			...
			AND id_site='" . sql_quote($crawltidsite) . "'";
        $crawltrequetecheck = faireUneRequeteOnLine_Crawltrack($crawltsqlcheck, $connexion);
        $nbrresultcheck = exploiterNombreLigneResultatBDD_Crawltrack($crawltrequetecheck);
        if ($nbrresultcheck >= 1) {
            $crawltligne = exploiterLigneResultatBDD_Crawltrack($crawltrequetecheck);
...
$requetegraph = faireUneRequeteOnLine_Crawltrack($sqlgraph, $connexion);
