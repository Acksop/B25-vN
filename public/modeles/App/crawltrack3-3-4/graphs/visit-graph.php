...
if (file_exists("../cachecloseperiod/$graphname.gz")) {
    ...
    $requete = faireUneRequeteOnLine_Crawltrack($sql, $connexion) or die("MySQL query error");
    $nbrresult = exploiterNombreLigneResultatBDD_Crawltrack($requete);
    if ($nbrresult >= 1) {
        $ligne = exploiterLigneResultatBDD_Crawltrack($requete);
...
$group->add($plot);
if ($navig == 4 || $navig == 21 || $graphvisithours == 1) {} else {
    // pages viewed
    if ($navig == 2) {
...
if ($navig == 2 || $navig == 17 || $navig == 18 || $navig == 19 || $navig == 21 || $graphvisithours == 1) {} else {
    // crawler
    if ($navig == 4) {
        $plot = new BarPlot($crawl, 2, 2);
    } else {
