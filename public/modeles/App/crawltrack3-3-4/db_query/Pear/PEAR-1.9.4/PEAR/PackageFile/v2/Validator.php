...
class PEAR_PackageFile_v2_Validator
{
    ...
    function validate(&$pf, $state = PEAR_VALIDATE_NORMAL)
    {
        ...
        if (isset($test['dependencies']) && isset($test['dependencies']['required']) && isset($test['dependencies']['required']['pearinstaller']) && isset($test['dependencies']['required']['pearinstaller']['min']) && version_compare('1.9.4', $test['dependencies']['required']['pearinstaller']['min'], '<')) {
    ...
    function _pearVersionTooLow($version)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'version' => $version
        ), 'This package.xml requires PEAR version %version% to parse properly, we are ' . 'version 1.9.4');
    }
        ...
        if (isset($php_errormsg)) {
            if (isset($this->_stack)) {
                ...
                $this->_stack->push(__FUNCTION__, 'warning', array(
                    'file' => $file,
                    'package' => $pn
                ), 'in %file%: Could not process file for unkown reasons,' . ' possibly a PHP parse error in %file% from %package%');
            }
        }
        /*
