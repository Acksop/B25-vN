...
class PEAR_PackageFile
{
     ...
     * @param string $file
     *            full path to the package.xml file (and the files
     *            it references)
     * @param string $archive
     *            optional name of the archive that the XML was
    ...
    function &fromXmlString($data, $state, $file, $archive = false)
    {
        if (preg_match('/<package[^>]+version=[\'"]([0-9]+\.[0-9]+)[\'"]/', $data, $packageversion)) {
            ...
            PEAR_ErrorStack::staticPush('PEAR_PackageFile', PEAR_PACKAGEFILE_ERROR_NO_PACKAGEVERSION, 'warning', array(
                'xml' => $data
            ), 'package.xml "' . $file . '" has no package.xml <package> version');
     ...
     * @return PEAR_PackageFile_v1|PEAR_PackageFile_v2 @using Archive_Tar to extract the files
    ...
    function &fromTgzFile($file, $state)
    {
        ...
        $this->_extractErrors();
        PEAR::staticPushErrorHandling(PEAR_ERROR_CALLBACK, array(
            $this,
            '_extractErrors'
        ));
        
        if (! $xml || ! $tar->extractList(array(
            $xml
        ), $tmpdir)) {
            $extra = implode("\n", $this->_extractErrors());
            if ($extra) {
     ...
     * @param int $state
     *            package state (one of PEAR_VALIDATE_* constants)
     * @param string|false $archive
     *            name of the archive this package.xml came
     *            from, if any
     * @return PEAR_PackageFile_v1|PEAR_PackageFile_v2
