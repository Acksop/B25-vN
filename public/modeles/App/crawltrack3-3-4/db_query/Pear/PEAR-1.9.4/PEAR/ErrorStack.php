...
class PEAR_ErrorStack
{
     ...
     * For PHP-related errors, this will be a file
     ...
     * The return value will be used to determine
    ...
    function &singleton($package, $msgCallback = false, $contextCallback = false, $throwPEAR_Error = false, $stackClass = 'PEAR_ErrorStack')
    {
        ...
        if (! class_exists($stackClass)) {
            ...
            PEAR_ErrorStack::staticPush('PEAR_ErrorStack', PEAR_ERRORSTACK_ERR_NONCLASS, 'exception', array(
                'stackclass' => $stackClass
            ), 'stack class "%stackclass%" is not a valid class name (should be like PEAR_ErrorStack)', false, $trace);
        }
     ...
     * @param string $msg
     *            Error message, or a portion of it if the message
     *            is to be generated
    ...
    function push($code, $level = 'error', $params = array(), $msg = false, $repackage = false, $backtrace = false)
    {
        ...
        if ($this->_msgCallback) {
            $msg = call_user_func_array($this->_msgCallback, array(
                &$this,
                $err
            ));
            $err['message'] = $msg;
     ...
     * @param string $level
     *            Set to a level name in order to retrieve only errors of a particular level
     * @param boolean $merge
     *            Set to return a flat array, not organized by package
     * @param array $sortfunc
     *            Function used to sort a merged array - default
            ...
            foreach ($err['params'] as $name => $val) {
                ...
                if (is_object($val)) {
                    if (method_exists($val, '__toString')) {
                        ...
                        PEAR_ErrorStack::staticPush('PEAR_ErrorStack', PEAR_ERRORSTACK_ERR_OBJTOSTRING, 'warning', array(
                            'obj' => get_class($val)
