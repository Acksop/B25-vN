...
class DB_oci8 extends DB_common
{
     ...
     *
     * @param resource $stmt
     *            the prepared statement's resource
    ...
    function &execute($stmt, $data = array())
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $this->last_query = preg_replace("/:bind$i(?!\d)/", $this->quoteSmart($data[$key]), $this->last_query, 1);
            $i ++;
        }
        if ($this->autocommit) {
     ...
     *        
     * @see DB_common::raiseError(), DB_oci8::errorNative(), DB_oci8::errorCode()
     */
