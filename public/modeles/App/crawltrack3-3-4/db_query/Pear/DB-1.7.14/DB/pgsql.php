...
class DB_pgsql extends DB_common
{
    ...
    function simpleQuery($query)
    {
        ...
        if ($ismanip) {
            ...
            return DB_OK;
        } elseif (preg_match('/^\s*\(*\s*(SELECT|EXPLAIN|FETCH|SHOW|WITH)\s/si', $query)) {
     ...
     *        
     * @see DB_common::raiseError(), DB_pgsql::errorNative(), DB_pgsql::errorCode()
     */
     ...
     *
     * @param int $resource
     *            the PostgreSQL result identifier
     * @param int $num_field
     *            the field number
     *            
    ...
    function getSpecialQuery($type)
    {
        switch ($type) {
            ...
            case 'schema.views':
                return "SELECT schemaname || '.' || viewname from pg_views WHERE schemaname" . " NOT IN ('information_schema', 'pg_catalog')";
            case 'views':
