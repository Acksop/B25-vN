 ...
 *
 * @see DB_common::tableInfo() {@internal Since the TABLEINFO constants are bitwised, if more of them are
 ...
 * and removed using <kbd>^</kbd>.
 *
 * @see DB_common::setOption() {@internal Since the PORTABILITY constants are bitwised, if more of them are
...
class DB
{
     ...
     *
     * @param mixed $dsn
     *            the string "data source name" or array in the
    ...
    public static function connect($dsn, $options = array())
    {
        ...
        $classname = "DB_${type}";
        if (! class_exists($classname)) {
            $tmp = PEAR::raiseError(null, DB_ERROR_NOT_FOUND, null, null, "Unable to include the DB/{$type}.php" . " file for '" . DB::getDSNString($dsn, true) . "'", 'DB_Error', true);
            return $tmp;
        }
    ...
    {
        $manips = 'INSERT|UPDATE|DELETE|REPLACE|' . 'CREATE|DROP|' . 'LOAD DATA|SELECT .* INTO .* FROM|COPY|' . 'ALTER|GRANT|REVOKE|' . 'LOCK|UNLOCK';
     ...
     *
     * @param
     *            array|string the DSN to parse and format
     * @param
     *            boolean true to hide the password, false to include it
     * @return string
     */
    public static function getDSNString($dsn, $hidePassword)
    {
        ...
        $dsnString .= '://' . $dsnArray['username'] . ':' . $dsnArray['password'] . '@' . $dsnArray['protocol'];
        if ($dsnArray['socket']) {
...
class DB_Error extends PEAR_Error
{
     ...
     * @param int $level
     *            what error level to use for $mode &
     *            PEAR_ERROR_TRIGGER
     * @param mixed $debuginfo
     *            additional debug info, such as the last query
     *            
     * @see PEAR_Error
     */
    ...
    public function __call($method, $arguments)
    {
        ...
        trigger_error('Call to undefined method DB_Error::' . $method . '()', E_USER_ERROR);
    }
    // }}}
}
...
class DB_result
{
     ...
     * @param array $options
     *            an associative array with result options
     *            
     * @return void
     */
     ...
     *
     * @param int $fetchmode
     *            the constant indicating how to format the data
     * @param int $rownum
     *            the row number to fetch (index starts at 0)
     *            
    ...
    function numRows()
    {
         ...
         * DB_Result object.
         */
        if (($this->dbh->features['limit'] === 'emulate' && $this->limit_from !== null) || $this->dbh->phptype == 'fbsql') {
