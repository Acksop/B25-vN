...
class PEAR_PackageFile_Generator_v1
{
     ...
     *            PEAR_Packager
     * @param
     *            bool if true, a .tgz is written, otherwise a .tar is written
     * @param
     *            string|null directory in which to save the .tgz
     ...
     *
     * @param array $pkginfo
     *            array with release information
     * @param bool $changelog
     *            whether the result will be in a changelog element
     *            
     * @return string XML data
     *        
     * @access private
    ...
    function &toV2($class = 'PEAR_PackageFile_v2', $strict = false)
    {
        ...
        if ($this->_packagefile->getConfigureOptions()) {
            ...
            foreach ($arr[$release]['configureoption'] as $i => $opt) {
                $arr[$release]['configureoption'][$i] = array(
                    'attribs' => $opt
                );
            }
    ...
    function _convertDependencies2_0(&$release, $internal = false)
    {
        ...
        if (! isset($release['dependencies']['required']['php'])) {
            $release['dependencies']['required']['php'] = array(
                'min' => '4.0.0'
            );
        }
        $order = array();
            ...
            if (count($package['platform'])) {
                ...
                foreach ($oses as $os => $releaseNum) {
                    ...
                    foreach ($package['install-as'] as $file => $as) {
                        // o <install as=..> tags for <file name=... install-as=...>
                        if (! isset($package['platform'][$file])) {
                            $release[$releaseNum]['filelist']['install'][] = array(
                                'attribs' => array(
                                    'name' => $file,
                                    'as' => $as
                                )
                        ...
                        // <file name=... platform=!this platform install-as=..>
                        if (isset($package['platform'][$file]) && $package['platform'][$file] == "!$os") {
                            $release[$releaseNum]['filelist']['ignore'][] = array(
                                'attribs' => array(
                                    'name' => $file
                                )
                            );
                ...
                if (count($generic) || count($genericIgnore)) {
                    ...
                    if (count($generic)) {
                        foreach ($generic as $file) {
                            ...
                            $release[count($oses)]['filelist']['install'][] = array(
                                'attribs' => array(
                                    'name' => $file,
                                    'as' => $installas
                                )
                    ...
                    if (count($genericIgnore)) {
                        foreach ($genericIgnore as $file) {
                            $release[count($oses)]['filelist']['ignore'][] = array(
                                'attribs' => array(
                                    'name' => $file
                                )
                            );
                ...
                foreach ($package['install-as'] as $file => $value) {
                    $release['filelist']['install'][] = array(
                        'attribs' => array(
                            'name' => $file,
                            'as' => $value
                        )
