...
class PEAR_PackageFile_v2_Validator
{
    ...
    function validate(&$pf, $state = PEAR_VALIDATE_NORMAL)
    {
        ...
        $structure = array(
            ...
            '*providesextension',
            '*srcpackage|*srcuri',
            '+phprelease|+extsrcrelease|+extbinrelease|' . '+zendextsrcrelease|+zendextbinrelease|bundle', // special validation needed
            '*changelog'
        );
        $test = $this->_packageInfo;
        if (isset($test['dependencies']) && isset($test['dependencies']['required']) && isset($test['dependencies']['required']['pearinstaller']) && isset($test['dependencies']['required']['pearinstaller']['min']) && '1.10.1' != '@package' . '_version@' && version_compare('1.10.1', $test['dependencies']['required']['pearinstaller']['min'], '<')) {
        ...
        if (! $this->_stack->hasErrors()) {
            ...
            if (PEAR::isError($chan)) {
                ...
                if (! $validator) {
                    ...
                    ), 'package "%channel%/%package%" cannot be properly validated without ' . 'validation package "%channel%/%name%-%version%"');
                    return $this->_isValid = 0;
                }
    ...
    function _validateStabilityVersion()
    {
        ...
        if ($a) {
            if (! preg_match('/^\d+(?:\.\d+)*(?:[a-zA-Z]+\d*)?\\z/', $this->_packageInfo['version']['release'])) {
    ...
    function _validatePhpDep($dep, $installcondition = false)
    {
        ...
        if (isset($dep['min'])) {
            if (! preg_match('/^\d+(?:\.\d+)*(?:[a-zA-Z]+\d*)?(?:-[a-zA-Z0-9]+)?\\z/', $dep['min'])) {
                $this->_invalidVersion($type . '<min>', $dep['min']);
            }
        }
        if (isset($dep['max'])) {
            if (! preg_match('/^\d+(?:\.\d+)*(?:[a-zA-Z]+\d*)?(?:-[a-zA-Z0-9]+)?\\z/', $dep['max'])) {
        ...
        if (isset($dep['exclude'])) {
            ...
            foreach ($dep['exclude'] as $exclude) {
                if (! preg_match('/^\d+(?:\.\d+)*(?:[a-zA-Z]+\d*)?(?:-[a-zA-Z0-9]+)?\\z/', $exclude)) {
    ...
    function _validatePearinstallerDep($dep)
    {
        ...
        if (isset($dep['exclude'])) {
            ...
            foreach ($dep['exclude'] as $exclude) {
                if (! preg_match('/^\d+(?:\.\d+)*(?:[a-zA-Z]+\d*)?\\z/', $exclude)) {
                            ...
                            foreach ($roles as $role) {
                                if ($role['role'] = $file['attribs']['role']) {
                                    ...
                                    if (isset($role['uri'])) {
                                        ...
                                        $params = array(
                                            'role' => $role['role'],
                                            'package' => $this->_pf->_registry->parsedPackageNameToString(array(
    ...
    function _pearVersionTooLow($version)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'version' => $version
        ), 'This package.xml requires PEAR version %version% to parse properly, we are ' . 'version 1.10.1');
    }
    ...
    function _ignoreNotAllowed($type)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'type' => $type
        ), '<%type%> is not allowed inside global <contents>, only inside ' . '<phprelease>/<extbinrelease>/<zendextbinrelease>, use <dir> and <file> only');
    }
    ...
    function _fileNotAllowed($type)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'type' => $type
        ), '<%type%> is not allowed inside release <filelist>, only inside ' . '<contents>, use <ignore> and <install> only');
    }

    function _oldStyleFileNotAllowed()
    {
        $this->_stack->push(__FUNCTION__, 'error', array(), 'Old-style <file>name</file> is not allowed.  Use' . '<file name="name" role="role"/>');
    }
    ...
    function _filelistCannotContainFile($filelist)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'tag' => $filelist
        ), '<%tag%> can only contain <dir>, contains <file>.  Use ' . '<dir name="/"> as the first dir element');
    }
    ...
    function _filelistMustContainDir($filelist)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'tag' => $filelist
        ), '<%tag%> must contain <dir>.  Use <dir name="/"> as the ' . 'first dir element');
    }
    ...
    function _UrlOrChannel($type, $name)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'type' => $type,
            'name' => $name
        ), 'Required dependency <%type%> "%name%" can have either url OR ' . 'channel attributes, and not both');
    }
    ...
    function _NoChannel($type, $name)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'type' => $type,
            'name' => $name
        ), 'Required dependency <%type%> "%name%" must have either url OR ' . 'channel attributes');
    }
    ...
    function _UrlOrChannelGroup($type, $name, $group)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            ...
            'name' => $name,
            'group' => $group
        ), 'Group "%group%" dependency <%type%> "%name%" can have either url OR ' . 'channel attributes, and not both');
    }
    ...
    function _NoChannelGroup($type, $name, $group)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            ...
            'name' => $name,
            'group' => $group
        ), 'Group "%group%" dependency <%type%> "%name%" must have either url OR ' . 'channel attributes');
    }
    ...
    function _invalidState($type, $value)
    {
        ...
        $this->_stack->push(__FUNCTION__, 'error', array(
            ...
            'value' => $value,
            'types' => $states
        ), 'Stability type <%type%> is not a valid stability (%value%), must be one of ' . '%types%');
    }
    ...
    function _subpackagesCannotConflict($name)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'name' => $name
        ), 'Subpackage dependency "%name%" cannot use <conflicts/>, ' . 'only package dependencies can use this tag');
    }
    ...
    function _conflictingDepsCannotHaveVersioning($type)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'type' => $type
        ), '%type%: conflicting dependencies cannot have versioning info, use <exclude> to ' . 'exclude specific versions of a dependency');
    }
    ...
    function _DepchannelCannotBeUri($type)
    {
        $this->_stack->push(__FUNCTION__, 'error', array(
            'type' => $type
        ), '%type%: channel cannot be __uri, this is a pseudo-channel reserved for uri ' . 'dependencies only');
    }
    ...
    function _analyzeBundledPackages()
    {
        ...
        $log = isset($this->_pf->_logger) ? array(
            &$this->_pf->_logger,
            'log'
        ) : array(
            $common,
        ...
        foreach ($info as $package) {
            ...
            call_user_func_array($log, array(
                1,
                "Analyzing bundled package $package"
            ));
            ...
            if (PEAR::isError($ret)) {
                call_user_func_array($log, array(
                    0,
                    "ERROR: package $package is not a valid " . 'package'
                ));
                $inf = $ret->getUserInfo();
                if (is_array($inf)) {
                    foreach ($inf as $err) {
        ...
        if (isset($php_errormsg)) {
            if (isset($this->_stack)) {
                ...
                $this->_stack->push(__FUNCTION__, 'warning', array(
                    'file' => $file,
                    'package' => $pn
                ), 'in %file%: Could not process file for unknown reasons,' . ' possibly a PHP parse error in %file% from %package%');
            }
        }
        /*
