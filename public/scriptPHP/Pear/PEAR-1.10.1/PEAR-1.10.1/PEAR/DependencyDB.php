...
class PEAR_DependencyDB
{
    ...
    /**
     * Get a raw dependency database.
     * Calls setConfig() and assertDepsDB()
     * 
     * @param
     *            PEAR_Config
     * @param
     *            string|false full path to the dependency database, or false to use default
    ...
    /**
     * Create the dependency database, if it doesn't exist.
     * Error if the database is
    ...
    function getDependentPackageDependencies(&$pkg)
    {
        ...
        foreach ($depend as $info) {
            ...
            foreach ($temp as $dep) {
                if (isset($dep['dep'], $dep['dep']['channel'], $dep['dep']['name']) && strtolower($dep['dep']['channel']) == $channel && strtolower($dep['dep']['name']) == $package) {
    ...
    function _dependsOn($parent, $child, &$checked)
    {
        ...
        foreach ($this->_cache['dependencies'][$channel][$package] as $info) {
            ...
            if (strtolower($info['dep']['channel']) == $depchannel && strtolower($info['dep']['name']) == $deppackage) {
                return true;
            }
        }
        ...
        foreach ($data['dependencies'][$channel][$package] as $dep) {
            ...
            if ($found) {
                ...
                if (! count($data['packages'][$depchannel][$depname])) {
                    ...
                    $data['packages'][$depchannel][$depname] = array_values($data['packages'][$depchannel][$depname]);
                }
            }
        }
    ...
    function _setPackageDeps(&$data, &$pkg)
    {
        ...
        if (isset($deps['required']['subpackage'])) {
            if (! isset($deps['required']['subpackage'][0])) {
                $deps['required']['subpackage'] = array(
                    $deps['required']['subpackage']
                );
            }
        ...
        if (isset($deps['optional']['subpackage'])) {
            if (! isset($deps['optional']['subpackage'][0])) {
                $deps['optional']['subpackage'] = array(
                    $deps['optional']['subpackage']
                );
            }
     ...
     * @param
     *            required|optional whether this is a required or an optional dep
     * @param
     *            string|false dependency group this dependency is from, or false for ordinary dep
     */
