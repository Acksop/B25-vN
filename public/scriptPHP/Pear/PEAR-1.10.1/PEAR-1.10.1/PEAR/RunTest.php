...
class PEAR_RunTest
{
    ...
    function run($file, $ini_settings = array(), $test_number = 1)
    {
        ...
        if (! empty($section_text['POST']) || ! empty($section_text['POST_RAW']) || ! empty($section_text['UPLOAD']) || ! empty($section_text['GET']) || ! empty($section_text['COOKIE']) || ! empty($section_text['EXPECTHEADERS'])) {
            if (empty($this->_options['cgi'])) {
                ...
                if (isset($this->_options['tapoutput'])) {
                    return array(
                        'ok',
                        ' # skip --cgi option needed for this test, "pear help run-tests" for info'
                    );
                }
                return 'SKIPPED';
            }
        ...
        if (isset($this->_options['coverage']) && $this->xdebug_loaded) {
            ...
            if (substr($lines[0], 0, 2) == '<?' || substr($lines[0], 0, 5) == '<?php') {
                unset($lines[0]);
            }
        ...
        do {
            if (isset($section_text['EXPECTF']) || isset($section_text['EXPECTREGEX'])) {
                ...
                if (isset($section_text['EXPECTFILE'])) {
                    ...
                    if (! ($fp = @fopen($f, 'rb'))) {
                        return PEAR::raiseError('--EXPECTFILE-- section file ' . $f . ' not found');
                    }
                    fclose($fp);
        ...
        // write .diff
        $returns = isset($section_text['RETURNS']) ? array(
            trim($section_text['RETURNS']),
            $return_value
        ) : null;
        $expectf = isset($section_text['EXPECTF']) ? $wanted_re : null;
...
";
    ...
    function generate_diff($wanted, $output, $rvalue, $wanted_re)
    {
        ...
        foreach ($w1 as $idx => $val) {
            if (! $wanted_re || ! isset($wr[$idx]) || ! isset($o1[$idx]) || ! preg_match('/^' . $wr[$idx] . '\\z/', $o1[$idx])) {
                $w2[sprintf("%03d<", $idx)] = sprintf("%03d- ", $idx + 1) . $val;
            }
        }
        foreach ($o1 as $idx => $val) {
            if (! $wanted_re || ! isset($wr[$idx]) || ! preg_match('/^' . $wr[$idx] . '\\z/', $val)) {
...
";
    ...
    {
        $this->headers = array();
        if (! empty($this->_options['cgi']) && $this->_php == $this->_options['cgi'] && preg_match("/^(.*?)(?:\n\n(.*)|\\z)/s", $output, $match)) {
