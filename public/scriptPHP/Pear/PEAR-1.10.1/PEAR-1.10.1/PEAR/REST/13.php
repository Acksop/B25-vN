...
class PEAR_REST_13 extends PEAR_REST_10
{
        ...
        foreach ($info['r'] as $release) {
            ...
            if (isset($state)) {
                ...
                if ($release['v'] == $version) {
                    if (! isset($this->_rest->_options['force']) && ! isset($version) && version_compare($release['m'], phpversion(), '>')) {
                        // skip releases that require a PHP version newer than our PHP version
                        $skippedphp = $release;
                        continue;
                    }
    ...
    function listLatestUpgrades($base, $pref_state, $installed, $channel, &$reg)
    {
        ...
        foreach ($packagelist['p'] as $package) {
            ...
            $info = $this->_rest->retrieveData($base . 'r/' . strtolower($package) . '/allreleases2.xml', false, false, $channel);
