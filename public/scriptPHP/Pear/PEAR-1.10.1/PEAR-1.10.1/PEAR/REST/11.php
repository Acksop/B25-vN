...
class PEAR_REST_11
{
    ...
    function listCategory($base, $category, $info = false, $channel = false)
    {
        ...
        $url = sprintf($url, $base, urlencode($category));
        ...
        if (! is_array($packagelist['pi']) || ! isset($packagelist['pi'][0])) { // only 1 pkg
            $packagelist_pre = array(
                $packagelist['pi']
            );
        } else {
