...
if (getenv('PHP_PEAR_CFG_DIR')) {
    ...
    define('PEAR_CONFIG_DEFAULT_CFG_DIR', $PEAR_INSTALL_DIR . DIRECTORY_SEPARATOR . 'cfg');
}
...
if (getenv('PHP_PEAR_WWW_DIR')) {
    ...
    define('PEAR_CONFIG_DEFAULT_WWW_DIR', $PEAR_INSTALL_DIR . DIRECTORY_SEPARATOR . 'www');
}
...
if (getenv('PHP_PEAR_MAN_DIR')) {
    ...
    if (defined('PHP_MANDIR')) { // Added in PHP5.3.7
        ...
        define('PEAR_CONFIG_DEFAULT_MAN_DIR', PHP_PREFIX . DIRECTORY_SEPARATOR . 'local' . DIRECTORY_SEPARATOR . 'man');
    }
}
...
class PEAR_Config extends PEAR
{
    ...
    /**
     * Information about the configuration data.
     * Stores the type,
...
3: debug',
     ...
     * If you want to keep only one instance
    ...
    function readConfigFile($file = null, $layer = 'user', $strict = true)
    {
        ...
        if (! $this->_noRegistry && ($phpdir = $this->get('php_dir', $layer, 'pear.php.net'))) {
            $this->_registry[$layer] = new PEAR_Registry($phpdir, false, false, $this->get('metadata_dir', $layer, 'pear.php.net'));
    ...
    /**
     * The ftp server is set in {@link readFTPConfigFile()}.
     * It exists only if a
    ...
    function setInstallRoot($root)
    {
        ...
        if (($old != $root) && ! $this->_noRegistry) {
            foreach (array_keys($this->_registry) as $layer) {
                ...
                $this->_registry[$layer] = new PEAR_Registry($this->get('php_dir', $layer, 'pear.php.net'), false, false, $this->get('metadata_dir', $layer, 'pear.php.net'));
                $this->_registry[$layer]->setConfig($this, false);
                $this->_regInitialized[$layer] = false;
            }
        }
