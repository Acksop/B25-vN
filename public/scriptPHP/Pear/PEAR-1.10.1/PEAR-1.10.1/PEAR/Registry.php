...
class PEAR_Registry extends PEAR
{
 ...
 // XXX UNUSED
        ...
        if (! isset($this->_dependencyDB)) {
            ...
            if (! $initializing) {
                ...
                if (PEAR::isError($this->_dependencyDB)) {
                    ...
                    if (file_exists($this->_config->get('metadata_dir', null, 'pear.php.net') . DIRECTORY_SEPARATOR . '.depdb')) {
                        @unlink($this->_config->get('metadata_dir', null, 'pear.php.net') . DIRECTORY_SEPARATOR . '.depdb');
                    }
    ...
    /**
     * PEAR_Registry destructor.
     * Makes sure no locks are forgotten.
     *
     * @access private
     */
    ...
    function _assertStateDir($channel = false)
    {
        ...
        if (! file_exists($this->channelsdir)) {
            if (! file_exists($this->channelsdir . $ds . 'pear.php.net.reg') || ! file_exists($this->channelsdir . $ds . 'pecl.php.net.reg') || ! file_exists($this->channelsdir . $ds . 'doc.php.net.reg') || ! file_exists($this->channelsdir . $ds . '__uri.reg')) {
                $init = true;
            }
    ...
    function _assertChannelDir()
    {
        ...
        if (! file_exists($this->channelsdir . DIRECTORY_SEPARATOR . '.alias')) {
            ...
            if (! System::mkdir(array(
                '-p',
                $this->channelsdir . DIRECTORY_SEPARATOR . '.alias'
            ))) {
     ...
     *            donotuse
     * @param
     *            string Last-Modified HTTP tag from remote request
    ...
    function _packageInfo($package = null, $key = null, $channel = 'pear.php.net')
    {
        if ($package === null) {
            ...
            return array_map(array(
                &$this,
                '_packageInfo'
            ), $ps, array_fill(0, count($ps), null), array_fill(0, count($ps), $channel));
        }
    ...
    function checkFileMap($path, $package = false, $api = '1.0', $attrs = false)
    {
        if (is_array($path)) {
            ...
            $package = is_array($package) ? array(
                strtolower($package[0]),
    ...
    // {{{ apiVersion()
    /**
     * Get the expected API version.
     * Channels API is version 1.1, as it is backwards
     * compatible with 1.0
     * 
     * @return string
     */
