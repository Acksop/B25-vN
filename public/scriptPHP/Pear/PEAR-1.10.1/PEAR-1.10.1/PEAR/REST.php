...
class PEAR_REST
{
    ...
    function saveCacheFile($file, $contents)
    {
        ...
        if ($cachefile_fp !== false) { // create file
            ...
            if (OS_WINDOWS) {
                ...
                $not_symlink = $cachefile_lstat['mode'] == $cachefile_fstat['mode'] && $cachefile_lstat['ino'] == $cachefile_fstat['ino'] && $cachefile_lstat['dev'] == $cachefile_fstat['dev'] && $cachefile_fstat['nlink'] === 1;
            }
    ...
    /**
     * Efficiently Download a file through HTTP.
     * Returns downloaded file as a string in-memory
