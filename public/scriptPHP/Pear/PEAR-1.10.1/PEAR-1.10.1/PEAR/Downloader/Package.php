...
class PEAR_Downloader_Package
{
                ...
                if ($param->_installRegistry->packageExists($package, $channel)) {
                    ...
                    if (version_compare($packageVersion, $param->getVersion(), '==')) {
                        if (! isset($options['force']) && ! isset($options['packagingroot'])) {
                            ...
                            unset($info['state']);
                            if (! isset($options['soft'])) {
                                $param->_downloader->log(1, 'Skipping package "' . $param->getShortName() . '", already installed as version ' . $packageVersion);
                            }
                            $params[$i] = false;
                        }
                    } elseif (! isset($options['force']) && ! isset($options['upgrade']) && ! isset($options['soft']) && ! isset($options['packagingroot'])) {
                        $info = $param->getParsedPackage();
                        $param->_downloader->log(1, 'Skipping package "' . $param->getShortName() . '", already installed as version ' . $packageVersion);
                        $params[$i] = false;
                    }
        ...
        foreach (array(
            ...
            if (isset($deps['optional'][$packagetype])) {
                ...
                if (! isset($deps['optional'][$packagetype][0])) {
                    $deps['optional'][$packagetype] = array(
                ...
                foreach ($deps['optional'][$packagetype] as $dep) {
                    ...
                    if (! isset($options['alldeps'])) {
                        ...
                        if (! isset($options['soft'])) {
                            $this->_downloader->log(3, 'Notice: package "' . $this->_registry->parsedPackageNameToString($this->getParsedPackage(), true) . '" optional dependency "' . $this->_registry->parsedPackageNameToString(array(
                                'package' => $dep['name'],
                                'channel' => 'pear.php.net'
                            ), true) . '" will not be automatically downloaded');
                        }
        ...
        foreach ($deps as $dep) {
            ...
            if (isset($dep['type']) && $dep['type'] === 'pkg') {
                    ...
                    if (PEAR::isError($url)) {
                        ...
                        if ($obj !== null && $this->isInstalled($obj, $dep['rel'])) {
                            ...
                            if (isset($dep['optional']) && $dep['optional'] == 'yes') {
                                $this->_downloader->log(2, $this->getShortName() . ': Skipping optional dependency "' . $this->_registry->parsedPackageNameToString($dep, true) . '", no releases exist');
                                continue;
                            } else {
                                return $url;
                            }
                ...
                if (! isset($options['alldeps'])) {
                    if (isset($dep['optional']) && $dep['optional'] == 'yes') {
                        if (! isset($options['soft'])) {
                            $this->_downloader->log(3, 'Notice: package "' . $this->getShortName() . '" optional dependency "' . $this->_registry->parsedPackageNameToString(array(
                                'channel' => $chan,
                                'package' => $dep['name']
                            ), true) . '" will not be automatically downloaded');
                        }
                        $skipnames[] = $this->_registry->parsedPackageNameToString(array(
                            'channel' => $chan,
                            'package' => $dep['name']
                        ), true);
                        $nodownload = true;
                ...
                if (! isset($options['alldeps']) && ! isset($options['onlyreqdeps'])) {
                    if (! isset($dep['optional']) || $dep['optional'] == 'no') {
                        if (! isset($options['soft'])) {
                            $this->_downloader->log(3, 'Notice: package "' . $this->getShortName() . '" required dependency "' . $this->_registry->parsedPackageNameToString(array(
                                'channel' => $chan,
                                'package' => $dep['name']
                            ), true) . '" will not be automatically downloaded');
                        }
     ...
     *            array
     * @param
     *            bool ignore install groups - for final removal of dupe packages
     */
    ...
    public static function mergeDependencies(&$params)
    {
        ...
        foreach ($params as $i => $param) {
            ...
            foreach ($contents as $file) {
                ...
                if (! PEAR_Downloader_Package::willDownload($obj, array_merge($params, $newparams)) && ! $param->isInstalled($obj)) {
                    $newparams[] = $obj;
                }
            }
        }
        ...
        foreach ($params as $i => $param) {
            ...
            foreach ($param->_downloadDeps as $dep) {
                ...
                if (! PEAR_Downloader_Package::willDownload($dep, $merge) && ! $param->isInstalled($dep)) {
                    $newdeps[] = $dep;
                } else {
                    // var_dump($dep);
     ...
     * The original parameter will be modified to reflect this.
     * 
     * @param
    ...
    function _fromUrl($param, $saveparam = '')
    {
        if (! is_array($param) && (preg_match('#^(http|https|ftp)://#', $param))) {
            $options = $this->_downloader->getOptions();
            $this->_type = 'url';
            $callback = $this->_downloader->ui ? array(
                &$this->_downloader,
                '_downloadCallback'
            ) : null;
            ...
            if ($this->_rawpackagefile) {
                ...
                if (str_replace(array(
                    ...
                    '',
                    ''
                ), $packagexml) != str_replace(array(
                    "\n",
                    "\r"
                ), array(
                    '',
                    ''
                ), $this->_rawpackagefile)) {
    ...
    function _analyzeDownloadURL($info, $param, $pname, $params = null, $optional = false, $isdependency = false)
    {
            ...
            if (isset($this->_downloader->_options['force'])) {
                ...
                if (isset($info['php']) && $info['php']) {
                    $err = PEAR::raiseError('Failed to download ' . $this->_registry->parsedPackageNameToString(array(
                        'channel' => $pname['channel'],
                        'package' => $pname['package']
                    ), true) . ', latest release is version ' . $info['php']['v'] . ', but it requires PHP version "' . $info['php']['m'] . '", use "' . $this->_registry->parsedPackageNameToString(array(
                        'channel' => $pname['channel'],
                        'package' => $pname['package'],
                        'version' => $info['php']['v']
                    )) . '" to install', PEAR_DOWNLOADER_PACKAGE_PHPVERSION);
                    return $err;
                }
                ...
                if (isset($options['ignorepreferred_state'])) {
                    $err = PEAR::raiseError('Failed to download ' . $this->_registry->parsedPackageNameToString(array(
                        'channel' => $pname['channel'],
                        'package' => $pname['package']
                    ), true) . $vs . ', latest release is version ' . $info['version'] . ', stability "' . $info['info']->getState() . '", use "' . $this->_registry->parsedPackageNameToString(array(
                        'channel' => $pname['channel'],
                        'package' => $pname['package'],
                        'version' => $info['version']
                    )) . '" to install', PEAR_DOWNLOADER_PACKAGE_STATE);
                    return $err;
                }
        ...
        if (isset($info['deprecated']) && $info['deprecated']) {
            $this->_downloader->log(0, 'WARNING: "' . $this->_registry->parsedPackageNameToString(array(
                'channel' => $info['info']->getChannel(),
                'package' => $info['info']->getPackage()
            ), true) . '" is deprecated in favor of "' . $this->_registry->parsedPackageNameToString($info['deprecated'], true) . '"');
        }
