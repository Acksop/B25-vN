...
function connectes()
{
    ...
    if (exploiterNombreLigneResultatBDD($result) == 0) {
        // insertion
    ...
    if (exploiterNombreLigneResultatBDD($result) > 1) {
        $ips = array();
        while ($data = exploiterLigneResultatBDD_row($result)) {
            $ips[] = $data[0];
        }
...
function AfficheNbConnectes()
{
    ...
    if (isset($_SESSION['type_compte']) && $_SESSION['type_compte'] == 0) {
        ...
        if (! $result) {
            ...
            while ($connecte = exploiterLigneResultatBDD_row($result))
                echo $connecte[0] . '<br/>';
        }
        echo "</h6>";
        $result = faireUneRequeteOffLine("SELECT count(*) FROM connectes WHERE pseudo = ''");
        if ($result) {
            $visiteurs = exploiterLigneResultatBDD_row($result);
