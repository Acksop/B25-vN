...
function inscriptionSite($login, $pass, $repertoire, $courriel, $type, $date)
{
    ...
    $sql = "SELECT id_utilisateur FROM utilisateur WHERE pseudo = '" . $login . "' AND password = '" . $pass . "'";
    ...
    if ($type == '1') {
        ...
        if ($type == '2' || $type == '4' || $type == '6') {
            ...
            if ($type == '3' || $type == '5' || $type == '9') {
                ...
                if ($type == '7' || $type = '8') {
                    // TODO: faire les requete pour les maisons et les quidams
                }
    return $usr_id;
}
...
function recuperationNbNouveauxMessages()
{
    ...
    return exploiterNombreLigneResultatBDD($data);
}
...
function recuperationNbAnnoncesActivesUtilisateur($id_artiste)
{
    ...
    $req = faireUneRequeteOffLine($sql);
    return exploiterNombreLigneResultatBDD($req);
}
...
function AjouterLectureArticleAfficher($id)
{
    ...
    $data = exploiterNombreLigneResultatBDD(faireUneRequeteOffLine($sql));
    if ($data == 0) {
...
function ajouterBUZZArtiste($id, $type)
{
    ...
    $data = exploiterLigneResultatBDD_row($req2);
    return $data[0];
}
...
function recuperationNbDossiersArchiver($id_utilisateur)
{
    ...
    return exploiterNombreLigneResultatBDD(faireUneRequeteOffline($sql));
}
...
function ajouterLectureDossierAfficher($id_dossier)
{
    ...
    $data = exploiterLigneResultatBDD_row(faireUneRequeteOffline($sql));
    $nb = $data[0] + 1;
...
function recuperationAdresseCourrielDepuisArticleEnValidation($id_article)
{
    ...
    $req = faireUneRequeteOffLine($sql);
    $data2 = exploiterLigneResultatBDD_row($req);
    switch ($data2[0]) {
        case 1:
...
function recuperationIDArtisteOuArtisanFromPseudo($pseudo)
{
    $sql = "SELECT id_artiste FROM artistes WHERE pseudo = '" . HTML_ChaineDeCaracteres($pseudo) . "'";
    $req = faireUneRequeteOffline($sql);
    $data = exploiterLigneResultatBDD_row($req);
    return $data[0];
}

function recuperationTypeCompteFromIDArtiste($id_artiste)
{
    $sql = "SELECT type_compte FROM utilisateur,artistes WHERE artistes.id_utilisateur = utilisateur.id_utilisateur AND artistes.id_artiste = '" . $id_artiste . "'";
    $req = faireUneRequeteOffline($sql);
    $data = exploiterLigneResultatBDD_row($req);
    return $data[0];
}
...
function modifierLogoArtisans($image, $largeur, $hauteur, $id)
{
    ...
    $req = faireUneRequeteOffLine($sql);
    $data = exploiterNombreLigneResultatBDD($req);
    if ($data == 1) {
...
function recuperationNbUtilisateuraReorganiser()
{
    ...
    $countID = exploiterNombreLigneResultatBDD($data);
    return $countID;
}
...
function recuperationIDBase()
{
    ...
    $data = faireUneRequeteOffline($sql);
    $data_final = exploiterLigneResultatBDD_row($data);
    return $data_final[0];
}
...
function AfficherLesComptesRelies($id_base)
{
    ...
    $resultat = faireUneRequeteOffLine($sql);
    echo "<ul>";
    while ($data = exploiterLigneResultatBDD_row($resultat)) {
        if ($data[0] == $id_base) {
        ...
        $req = faireUneRequeteOffLine($sql);
        $res = exploiterLigneResultatBDD_row($req);
        if (($data[0] == $id_base && $data[1] == - 1) || ($data[1] == $id_base && $data[0] == - 1)) {
            echo "<li>Cet accès aux pleins-pouvoir par lequel vous êtes arrivé n'est plus accessible, par contre une personne accédant de la même façon que vous, y accède avec les même pouvoirs donc vous êtes sous le joug de cette personne si vous ne supprimez pas cette liaison ! &gt;&gt;&gt;<span style='float:right;'><a href='controlleurs/traitementSuppressionLiaisonCompte.php?id1=" . $data[0] . "&id2=" . $data[1] . "'>[X]</a></span></li>";
        } else {
...
function aCompteRelier($id_base)
{
    return estCompteRelier($id_base);
}
...
function existeLiaisonCompte($id_compte1, $id_compte2)
{
    $sql1 = "SELECT * FROM estRelierA WHERE (idCompte1 = '" . $id_compte1 . "' && idCompte2 = '" . $id_compte2 . "')";
    $sql2 = "SELECT * FROM estRelierA WHERE (idCompte2 = '" . $id_compte1 . "' && idCompte1 = '" . $id_compte2 . "')";
    
    $req1 = faireUneRequeteOffLine($sql1);
    $req2 = faireUneRequeteOffLine($sql2);
    
    if (exploiterNombreLigneResultatBDD($req1) > 0) {
        return TRUE;
    } else {
        if (exploiterNombreLigneResultatBDD($req2) > 0) {
            return TRUE;
        }
        return FALSE;
    }
...
function afficheLienConnexionTransparente($id)
{
    ...
    return $pageB25FR . "controlleurs/traitementIdentificationAutomatique.php?l={$l}&m={$m}";
}
