...
function AfficheCalendrier($annee, $mois)
{
    ...
    for ($i = 0; $i <= $nbsem - 1; $i ++) {
        ...
        for ($j = 0; $j < 7; $j ++) {
            ...
            // recherche de la base de donn�es des enregistrements
            ...
            echo "<td align='center'" . (($jourCourant['mon'] == $mois) ? " class='mois' " : " class='grise' ") . (($data[0] != NULL && $jourCourant['mon'] == $mois) ? " background='images/box_clear.gif' " : "") . "><a href='index.php?Mois=" . $jourCourant['mon'] . "&Annee=" . $jourCourant['year'] . "&Jour=" . $jourCourant['mday'] . "'>" . $jourCourant['mday'] . "</a></td>";
        }
        echo "</tr>";
    }
    echo "</table>";
...
function transformationDateArticlePourRSS($dateArticle)
{
    ...
    if ($tabDate[1] === "Mercredi") {
        $jour = "Wed";
    }
    if ($tabDate[1] === "Jeudi") {
        $jour = "Thu";
    }
    if ($tabDate[1] === "Vendredi") {
        $jour = "Fri";
    }
    if ($tabDate[1] === "Samedi") {
        $jour = "Sat";
    }
    if ($tabDate[1] === "Dimanche") {
        $jour = "Sun";
    }
    ...
    $mois = "Oct";
    if ($tabDate[3] === "Janvier") {
        $mois = "Jan";
    }
    if ($tabDate[3] === "F&eacute;vrier") {
        $mois = "Feb";
    }
    if ($tabDate[3] === "Mars") {
        $mois = "Mar";
    ...
    if ($tabDate[3] === "Juillet") {
        $mois = "Jul";
    }
    if ($tabDate[3] === "Ao&ucirc;t") {
        $mois = "Aug";
    }
    if ($tabDate[3] === "Septembre") {
        $mois = "Sep";
    }
    if ($tabDate[3] === "Octobre") {
        $mois = "Oct";
    }
    if ($tabDate[3] === "Novembre") {
        $mois = "Nov";
    }
    if ($tabDate[3] === "D&eacute;cembre") {
        $mois = "Dec";
    }
    // transformation du Jour
    $annee = $tabDate[4];
