...
// Une balise WCode est compos� de 5 choses : nom, valeur, contenu, code correspondant & balise ferm�e
class wcode_element
{
 ...
 // Valeur de la balise
                      ...
                      // Pour les effray�s de la recursivit�, sachez qu'il s'arr�te � la balise la plus �lementaire : la balise P
    var $contenu = '';
 // Contenu de la balise
    var $code_html = '';
 // Code HTML avec %VALEUR% et %CONTENU% ou %FONCTION%
    var $erreur = '';
 // Message en cas d'erreur
    var $ferme = false;

    var $retour_ligne = false;
    
    // definir : Chaine, Chaine, Chaine, Chaine --> Rien
    // D�finit les valeurs du wcode_element
    function definir($n, $v, $c, $ch)
    {
        $this->nom = $n;
        $this->valeur = $v;
        $this->contenu = $c;
    ...
    {
        if ($this->nom == 'p') {
            if ($this->retour_ligne) {
                $this->code_html = stripslashes($this->contenu);
                $this->code_html = nl2br($this->code_html);
            ...
            $retour = str_replace(Array(
                ...
                htmlentities(stripslashes($this->valeur)),
                $this->contenu->donner_html()
            ), $this->code_html);
...
class wcode
{
    ...
    var $status_smileys = false;
 // Status des smileys
    var $status_balises = false;
 // Status des balises
    var $smileys = Array();
 // Liste des smileys
    var $balises = Array();
 // Liste des balises
    var $code = '';
 // WCode brut
    var $liste_elements = Array();
 // Liste des �l�ments
    var $code_travail = '';
 // WCode de travail : D�truit au fur et � mesure de la lecture
    var $code_html = '';
 // Le code HTML
    var $erreur = '';
 // Erreur renvoy�e
    var $code_faux = '';
 // Code HTML pour visualiser l'erreur
    var $retour_ligne = true;
 // Retour � la ligne !?
                              
    // erreur : Chaine, Chaine ----------> Rien
                              // Signaler une erreur et mettre en �vidence le code faux
    function erreur($message, $code_faux)
    {
        $this->erreur = $message;
        $p = strpos($this->code, $code_faux);
        $this->code_faux = nl2br(htmlentities(substr($this->code, 0, $p)) . '<span id="fluo">' . htmlentities($code_faux) . '</span>' . htmlentities(substr($this->code, $p + strlen($code_faux))));
    }
    // status_smileys : Bool�en -----------> Rien
    // D�finit le status des smileys
    function status_smileys($s)
    {
        $this->status_smileys = $s;
    }
    ...
    function ct_len()
    {
        return strlen($this->code_travail);
    }
    
    // ct_retirer : Entier(i) -----------> Chaine
    // Retire les i premiers caract�res de $code_travail et les renvoie
    ...
    // Renvoie les i premiers caract�res de $code_travail
    function ct_lire($i)
    {
        return substr($this->code_travail, 0, $i);
    }
    
    // status_balises : Bool�en -----------> Rien
    // D�finit le status des balises
    function status_balises($s)
    {
        $this->status_balises = $s;
    }
    
    // definir_code : Cha�ne -----------> Rien
    // D�finit le WCode � analyser
    function definir_code($c)
    {
        $this->code = $c;
    }
    
    // donner_html : Rien -------------> Cha�ne
    ...
    {
        if (! file_exists(dirname(__FILE__) . DIRECTORY_SEPARATOR . $fichier))
            return false;
        include_once (dirname(__FILE__) . DIRECTORY_SEPARATOR . $fichier);
    ...
    // lire_code : Rien -----------> Bool�en
    // Renvoie le r�sultat de l'analyse du code
    function lire_code()
    {
        // Nous voici donc dans la partie principale du programme
        // Ici, nous n'avons que la variable $code ainsi que les param�tres de configuration
        // Nous allons la copier dans une variable de travail
        $this->code_travail = $this->code;
        
        // Commen�ons par remplacer tous les smileys par leurs �quivalents image
        ...
        $this->code_html = '';
        foreach ($this->liste_elements as $e) {
            $this->code_html .= $e->donner_html();
        }
        return true;
    }
    ...
    function lire_texte()
    {
        ...
        while ($this->ct_lire(1) != '[' && $this->ct_len() != 0) {
            if ($this->ct_lire(1) === '\\')
                $contenu .= $this->ct_retirer(2);
            else
                $contenu .= $this->ct_retirer(1);
        }
    ...
    // Pr�l�ve du code de travail la balise et la renvoie en wcode_element
    function lire_balise()
    {
        ...
        while ($this->ct_lire(1) != "=" && $this->ct_lire(1) != "]" && $this->ct_len() != 0) {
            if ($this->ct_lire(1) === '\\')
                $nom .= $this->ct_retirer(2);
            else
                $nom .= $this->ct_retirer(1);
        }
        ...
        // S'il s'est arr�t� sur un =, alors Il y a une valeur � lire
        if ($this->ct_lire(1) === '=') {
            $this->ct_retirer(1);
            while ($this->ct_lire(1) != "]" && $this->ct_len() != 0) {
                if ($this->ct_lire(1) === '\\')
                    $valeur .= $this->ct_retirer(2);
                else
                    $valeur .= $this->ct_retirer(1);
            }
            // Pourquoi s'est-il arr�t� ?
            if ($this->ct_len() === 0) {
                $obj = new wcode_element();
        ...
        // Ici, on va v�rifier que la balise existe, et le cas �ch�ant, v�rifier l'expression de la valeur
        $trouve = false;
        ...
        if ($ferme) {
            // Voici une petite explication simple de la technique utilis�e :
            // On va avancer dans le code caract�re par caract�re, jusqu'� temps d'avoir autant de balises
            // d'ouverture que de balises de fermeture !
            // Cat d'arr�t : $x === $this->ct_len();
            $balise_fermeture = '[/' . $nom . ']';
            if ($regex_valeur === '')
                $balise_ouverture = '\\[' . $nom . '\\]';
            else
                $balise_ouverture = '\\[' . $nom . '=' . $regex_valeur . '\\]';
            ;
            $x = strlen($balise_fermeture); // Commencer � 1 c'est inutile !
            ...
            // Toujours pas ferm� cette -*#N�p' de balise ?!
            if ($ouvertures != $fermetures) {
                $contenu = $this->ct_retirer($x);
                $obj = new wcode_element();
                $obj->erreur = "Une balise " . $nom . " n'est pas ferm�e !";
                if ($valeur != '')
                    $obj->code = '[' . $nom . '=' . $valeur . ']' . $contenu;
                else
                    $obj->code = '[' . $nom . ']' . $contenu;
                return $obj;
            }
        ...
        // En th�orie, ici, le contenu est bon !
        $obj = new wcode_element();
