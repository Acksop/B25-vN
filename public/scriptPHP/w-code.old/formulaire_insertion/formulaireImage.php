...
// ---- repertoire a ouvrir ---------------------------------
$NomRep = RADIEURAE_REP_PATH . "upload_utilisateurs/" . $_SESSION['repertoire'] . "/images";
$NomSVN = RADIEURAE_SVN_PATH . "upload_utilisateurs/" . $_SESSION['repertoire'] . "/images";
...
echo "</form>";
echo "</td></tr>";
echo "<tr><td>" . "<div class='formulaire' id='formulaire'>" . "<table border='0' height='100' width='200' align='center'>" . "<tr><td>" . "<p align ='center'>" . "<form enctype='multipart/form-data' method='post' action='TraitementInsertionImages.php'>" . "<input type='hidden' name='MAX_FILE_SIZE' value='2097152'/>" . "<p class='Titre'>Image � telecharger sur le SERVEUR:</p><input type='file' name='Fichier'/><BR>(max 2Mo)" . "<input type='submit' name='btnUpload' value='T�l�charger' onClick='AfficheFormTelechargement();'/>" . "</p>" . "</form>" . "</td></tr>" . "</table>" . "</div>" . "<div style='display:none' class='telechargementEnCours' id='telechargementEnCours'>" . "<table border='0' height='100' width='200' align='center'>" . "<tr><td>" . "<img name='telechargementEnCours' src='../../../images/upload.jpg' />" . "</td></tr>" . "</table>" . "</div>";
...
function AfficherImagesMiniatureEtSuppression($repertoire, $NomSVN)
{
    ...
    for ($i = 0; $i < sizeof($TabImagesRepCourant); $i ++) {
        // recherche it�rative de la miniature correspondante a l'image que l'on est en train de regarder.
        $existe = - 1;
        ...
        // si la miniature n'as pas �t� trouv�e au bout de l'it�ration
        if ($existe == - 1) {
            // on la cr��
        ...
        echo "<input type='radio' name='imageChoisie' value='" . $NomSVN . "/" . $TabImagesRepCourant[$i] . "'";
        if ($i == 0)
            echo "checked='checked'";
        echo " />";
        echo "</td><td>";
        echo "<a href='" . $repertoire . "/" . $TabImagesRepCourant[$i] . "'>" . "<IMG NAME='Apercu du fichier: " . $TabImagesRepCourant[$i] . "' SRC='" . $NomSVN . "/&MINI_" . $TabMiniRepCourant[$existe] . "' " . $ImgBalise[2] . " BORDER='0'>" . "</a>";
        echo "</td><td>";
