...
function listerContenuRepertoireImageArticle($NomRepertoire,$NomImageActive = '')
{
...
function creerRepertoiresUtilisateur($nomRep)
{
    ...
    $repertoire_utilisateur = RADIEURAE_REP_PATH . "upload_utilisateurs/" . $nomRep;
    $mode = 1777;
...
function recreerRepertoiresUtilisateur($nomRep)
{
    ...
    $mode = 0777;
    if (! @is_dir($repertoire_utilisateur))
        @mkdir($repertoire_utilisateur, $mode);
    if (! @is_dir($rep1))
        @mkdir($rep1, $mode);
    if (! @is_dir($rep2))
