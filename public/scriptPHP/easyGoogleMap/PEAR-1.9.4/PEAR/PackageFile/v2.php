...
class PEAR_PackageFile_v2
{
     ...
     *            array options
     * @param
     *            array package name as returned from {@link PEAR_Registry::parsePackageName()}
     * @param
     *            int PEAR_VALIDATE_* constant
                ...
                if (is_array($ret)) {
                    if (count($ret) == 1) {
                        ...
                        if (is_array($err)) {
                            ...
                            $installer->log(0, 'Download and install of binary extension "' . $this->_registry->parsedPackageNameToString(array(
    ...
    function getMaintainers($raw = false)
    {
        ...
        if ($raw) {
            ...
            $ret = array();
            $leads = isset($this->_packageInfo['lead'][0]) ? $this->_packageInfo['lead'] : array(
            ...
            if (isset($this->_packageInfo['developer'])) {
                $leads = isset($this->_packageInfo['developer'][0]) ? $this->_packageInfo['developer'] : array(
            ...
            if (isset($this->_packageInfo['contributor'])) {
                $leads = isset($this->_packageInfo['contributor'][0]) ? $this->_packageInfo['contributor'] : array(
            ...
            if (isset($this->_packageInfo['helper'])) {
                $leads = isset($this->_packageInfo['helper'][0]) ? $this->_packageInfo['helper'] : array(
    ...
    function getConfigureOptions()
    {
        ...
        if (isset($releases['configureoption'])) {
            if (! isset($releases['configureoption'][0])) {
                $releases['configureoption'] = array(
                    $releases['configureoption']
                );
            }
    ...
    function getInstallationFilelist($forfilecheck = false)
    {
        ...
        if ($release) {
            ...
            $depchecker = &$this->getPEARDependency2($this->_config, array(), array(
            ...
            foreach ($release as $instance) {
                ...
                if (isset($instance['filelist'])) {
                    ...
                    if (isset($instance['filelist']['ignore'])) {
                        $ignore = isset($instance['filelist']['ignore'][0]) ? $instance['filelist']['ignore'] : array(
                    ...
                    if (isset($instance['filelist']['install'])) {
                        $installas = isset($instance['filelist']['install'][0]) ? $instance['filelist']['install'] : array(
     ...
     * 
     * @param
     *            PEAR_PackageFile_v2|PEAR_PackageFile_v1|PEAR_Downloader_Package
     * @return boolean
     */
    ...
    function getDeps($raw = false, $nopearinstaller = false)
    {
        if (isset($this->_packageInfo['dependencies'])) {
            ...
            foreach (array(
                ...
                if (! isset($this->_packageInfo['dependencies'][$type]) || empty($this->_packageInfo['dependencies'][$type])) {
                    continue;
                }
    ...
    /**
     * Determine whether a task name is a valid task.
     * Custom tasks may be defined
