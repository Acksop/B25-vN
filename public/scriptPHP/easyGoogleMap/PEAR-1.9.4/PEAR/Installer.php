...
class PEAR_Installer extends PEAR_Downloader
{
    ...
    function _removeBackups($files)
    {
        foreach ($files as $path) {
            $this->addFileOperation('removebackup', array(
                $path
            ));
        }
    }
    ...
    function _deletePackageFiles($package, $channel = false, $backup = false)
    {
        ...
        foreach ($filelist as $file => $props) {
            ...
            if ($backup) {
                $this->addFileOperation('backup', array(
                    $path
                ));
                $ret[] = $path;
            }
            
            $this->addFileOperation('delete', array(
                $path
            ));
        }
    ...
    function _installFile($file, $atts, $tmp_path, $options)
    {
        ...
        list ($dest_file, $orig_file) = preg_replace(array(
            '!\\\\+!',
            '!/!',
            "!$ds2+!"
        ), array(
        ...
        if (empty($this->_options['register-only'])) {
            ...
            if (! OS_WINDOWS) {
                ...
                if ($atts['role'] != 'src') {
                    $this->addFileOperation("chmod", array(
                        $mode,
                        $dest_file
                    ));
     ...
     * @param array $data
     *            For all file operations, this array must contain the
            ...
            switch ($type) {
                ...
                case 'installed_as':
                    ...
                    if (! isset($this->_dirtree[dirname($data[1])])) {
                        ...
                        while (! empty($data[3]) && dirname($data[3]) != $data[3] && $data[3] != '/' && $data[3] != '\\') {
    ...
    {
        $this->addFileOperation('mkdir', array(
            $dir
        ));
     ...
     *
     * @param
     *            array a mixed list of package names, local files, or package.xml
     * @param
     *            PEAR_Config
     * @param
     *            array options from the command line
     * @param
     *            array this is the array that will be populated with packages to
     ...
     *            </code>
     * @param
     *            array this will be populated with any error messages
     * @param
     *            false private recursion variable
     * @param
     *            false private recursion variable
     * @param
     *            false private recursion variable
     ...
     *
     * @param string|PEAR_Downloader_Package $pkgfile
    ...
    function install($pkgfile, $options = array())
    {
        ...
        if (empty($options['upgrade'])) {
            ...
            if ($dirtree) {
                ...
                uksort($dirtree, array(
                    $this,
                    '_sortDirs'
                ));
                foreach ($dirtree as $dir => $notused) {
                    $this->addFileOperation('rmdir', array(
                        $dir
                    ));
                }
    ...
    function _compileSourceFiles($savechannel, &$filelist)
    {
        ...
        $built = $bob->build($filelist, array(
            &$this,
            '_buildCallback'
        ));
    ...
    function uninstall($package, $options = array())
    {
        ...
        if (! $pkg) {
            $this->configSet('default_channel', $savechannel);
            return $this->raiseError($this->_registry->parsedPackageNameToString(array(
                'channel' => $channel,
                'package' => $package
            ), true) . ' not installed');
        }
        ...
        $depchecker = &new PEAR_Dependency2($this->config, $options, array(
            'channel' => $channel,
            'package' => $package
