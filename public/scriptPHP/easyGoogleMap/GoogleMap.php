...
class GoogleMapAPI
{
    ...
    /**
     * PEAR::DB DSN for geocode caching.
     * example:
    ...
    /**
     * With this, you can append lang= and region= to the script url for localization.
     * If Google adds more features in the future, they will be supported by default
     *
    ...
    /**
     * Class variable that will store flag to minify js - this can be overwritten after object is instantiated.
     * Include JSMin.php if
     ...
     *
     * @param string $styles
     *            json string of the map styles to be applied
     */
     ...
     * @param string $icon_shadow_filename
    ...
    function getAddDirectionsJS()
    {
        ...
        foreach ($this->_directions as $directions) {
            ...
            if ($this->walking_directions == TRUE)
                ...
                if ($this->biking_directions == TRUE)
    ...
    function putCache($address, $lon, $lat)
    {
        ...
        $_res = & $_db->query('insert into ' . $this->_db_cache_table . ' values (?, ?, ?)', array(
            $address,
            $lon,
            $lat
        ));
    ...
    /**
     * fetch a URL.
     * Override this method to change the way URLs are fetched.
     *
     * @param string $url            
     */
